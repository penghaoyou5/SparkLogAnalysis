...
object WeblukerSqlQueryDict {
  val domainUserDict = scala.collection.mutable.Map[String,String]()
...
//          println(rs.getString("dstdomainName") + "   " + rs.getInt("userId"))
    ...
    domainUserDict(rs.getString("dstdomainName")) = rs.getInt("userId").toString
  }
  conn.close
  ...
  def getDomainUserIdDict(): scala.collection.mutable.Map[String, String] = {
    domainUserDict
  }
